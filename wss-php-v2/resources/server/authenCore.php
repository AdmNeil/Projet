<?php
  ...
  if(!empty($_POST['username']) && !empty($_POST['password'])){
    ...
    // rate limit by user they are trying to log into (limit+1, limit IP before user)
    $blk[1] = $cdir.'.loginratelimit-user-'.$user;
    $now = time();
    if (!$config['am']['authsec']['enabled'] or !is_file($blk[0]) or !is_file($blk[1]) or (max([filemtime($blk[0]), filemtime($blk[1])]) < $now)) {
      ...
      $pdo = query('am', 'SELECT password FROM authme WHERE username = ?', [$user]);
